...
test('QR page exposes request workflow and keeps destructive controls admin gated', async () => {
    ...
    assert.match(source, /<Card v-if="canRequestQr">[\s\S]*Submit Request/)
    assert.match(source, /<Card v-if="canRequestQr" class="mt-6">[\s\S]*QR Requests/)
    ...
    assert.match(source, /<Card v-if="canManageQr" class="mt-6">[\s\S]*Direct QR Issuance/)
    assert.match(source, /<Card v-if="canManageQr" class="mt-6">[\s\S]*Persisted QR Inventory/)
