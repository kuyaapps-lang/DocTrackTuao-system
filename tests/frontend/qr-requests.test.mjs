...
test('QR page exposes request workflow and keeps destructive controls admin gated', async () => {
    ...
    assert.match(source, /<Card v-if="canRequestQr && !canApproveQr" class="overflow-hidden border-blue-200">[\s\S]*Submit Request/)
    assert.match(source, /<Card v-if="canRequestQr" class="mt-6 overflow-hidden border-blue-200">[\s\S]*Office QR Requests/)
